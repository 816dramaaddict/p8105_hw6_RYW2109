...
cv_model = function(data, formula, cv_splits) {
  results = cv_splits %>%
    mutate(
      ...
      predictions = map2(model, splits, ~ predict(.x, new_data = assessment(.y))),
      residuals = map2(predictions, splits, ~ assessment(.x, assessment(.y)))
    ) %>%
    unnest(residuals) 
...
```{r}
model1_fitted = predict(model1, newdata = data_clean)
